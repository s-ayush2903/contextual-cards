...
/**
 * A `singleton`(not exactly, but equivalent) that instantiates retrofit and other related networking services that assist in fetching,
 * serializing, deserializing & Observing the data in the application.
 * By this singleton it is made sure that we do not create multiple instances of
 * networking services & retrofit which reduces memory usage and improves performance
 * as networking sits at the core of the application and instantiating core-services for each use will
 * definitely result in increment in greater consumption of resources
 */
class ApiDelegate {
