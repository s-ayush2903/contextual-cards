...
class CardViewHolder(viewBinding: ViewBinding) : RecyclerView.ViewHolder(viewBinding.root) {
     ...
     * Listener is configured on the Card ONLY if the card contains a @param [Card.url],
     * because in opposite case, it does not seem reasonable to have a listener on a View which is
     * just of no use(un-interactive), as it can't trigger or start a new action or change, and also clicking on that particular
     * view will (although not significantly, but still some)increase in memory load chunk of application
     * from the entire pie available to all apps and will consume resources, which do not seem to reasonable in this case.
     */
