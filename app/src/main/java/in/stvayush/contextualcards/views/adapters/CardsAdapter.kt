...
/**
 * An [Adapter] that inflates & binds cardLayouts It also takes care of itemView type and displaying
 * the menu accordingly It takes
 * @param [designType] in order to parse what type of card is obtained from server response and
 * bind/inflate layout accordingly
 * @param [groupId] in order to take note of the preference selected by user on HC3 card for
 * displaying in the next run or never
 */
class CardsAdapter(private val designType: DesignType, private val groupId: Long) :
      ...
      when (designType) {
        ...
        BIG_DISPLAY_CARD -> {
          if (viewType == SHOW_MENU) {
            layoutBigCardMenuBinding = LayoutBigCardMenuBinding.inflate(inflater, parent, false)
        ...
        DYNAMIC_WIDTH_CARD -> {
          layoutDynamicWidthCardBinding =
            LayoutDynamicWidthCardBinding.inflate(inflater, parent, false)
          layoutDynamicWidthCardBinding
        }
      }
  ...
  override fun onBindViewHolder(holder: CardViewHolder, position: Int) {
    ...
    with(cardItem) item@{
      with(holder) {
        when (designType) {
          SMALL_DISPLAY_CARD -> SmallCardViewHolder(layoutSmallCardBinding).bindSmallCard(this@item)
          BIG_DISPLAY_CARD -> {
