...
/**
 * [CardGroupViewModel] communicates to both, [MainActivity] & the [Repository] A networking call
 * received from [MainActivity] is triggered via instantiating the [Repository] and updating the
 * @param [errorMessage] to reflect the changes accordingly to the User. All this is monitored
 * easily as LiveData is being used here, which changes dynamically based on the response received
 * from the server
 */
class CardGroupViewModel : ViewModel() {
  ...
  /**
   * Kind of a fetching `status`, of how the communication with the server is going on It is set tu
   * true on 200 OK response, and set false otherwise, also the @param [errorMessage] too gets
   * updated
   */
  var fetchSuccessful: MutableLiveData<Boolean> = MutableLiveData()

  /**
   * A message to be displayed to the user as `help`, in cases of mis-happenings or poor
   * interaction(s) with the server
   */
  lateinit var errorMessage: String

  /**
   * A list of [CardGroup] to store and map the data received in the JSON form, from the Web API
   * response This mapping takes place via the data models that we have created for different
   * components being received from backend
   */
  ...
  fun fetchCards() {
    repository
      ...
      .subscribeWith(
        object : DisposableObserver<CardGroupObject>() {
  ...
  /**
   * Remove the [CardGroup] which user selected to not display, it works on the basis of Id of the
   * [CardGroup], so if user selects not to display a card with id `Xid` then it'll never be
   * displayed, no matter how many times refresh is triggered, as it would have stored the Id of the
   * [CardGroup] It will be visible only when existing local cache of application is cleared, or
   * practically, when the application is un-installed and then installed
   */
